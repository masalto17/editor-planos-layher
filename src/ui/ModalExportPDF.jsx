 ...
 * Campos: nombre, cliente, ubicación, plano Nº, revisión, escala.
 * Opciones: incluir planta, incluir corte transversal.
 */
export default function ModalExportPDF({ nombreActual, piezas, onExportar, onCerrar, exportando, tienePlanta }) {
  ...
  const [opcExpanded, setOpcExpanded] = useState(false);

  // Detectar posiciones X con verticales para sugerir cortes
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onCerrar}>
      <div className="bg-white rounded-lg shadow-xl w-[440px] max-w-[90vw] max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        {/* Header */}
        ...
        <div className="p-4 space-y-3">
          ...
          {/* Opciones avanzadas */}
          <button
            onClick={() => setOpcExpanded(!opcExpanded)}
            className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 mt-1"
          >
            {opcExpanded ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
            Opciones de páginas
          </button>
          ...
          <div className="text-[9px] text-gray-400 leading-tight mt-2">
            PDF A3 apaisado con membrete másalto/MYD, vistas del plano, despiece y sellos legales.
          </div>
        </div>
