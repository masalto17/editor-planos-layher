// Horizontal U T14 — perfil U reforzado para mayor capacidad de carga (ref 2618.xxx).
// Similar a HorizontalU pero más grueso, con etiqueta "T14" y doble línea de alma.
export default function HorizontalUT14({ pieza, worldToScreen, zoom, sc, op, cur, seleccionada, onMouseDown, modoTecnico }) {
  const { x, y, largo } = pieza;
  const pL = worldToScreen(x, y), pR = worldToScreen(x + largo, y);
  const w = pR.x - pL.x;
  const gid = `hut14g-${pieza.id}`;

  // Grosor más grueso que HorizontalU (reforzado)
  const g = Math.max(3.5, zoom * 0.065);
  const alaH = Math.max(4, zoom * 0.045);
  const hlOff = g * 0.2;

  // Cabezal cuña (igual proporción que HorizontalU pero un poco más grande)
  const mW = Math.max(6, zoom * 0.06);
  const mH = Math.max(8, zoom * 0.075);
  const wdW = Math.max(2.5, zoom * 0.025);
  const wdH = Math.max(4, zoom * 0.04);
  const r = Math.max(0.5, zoom * 0.004);
  const showDetail = zoom > 45;
  const showLabel = zoom > 40;

  const tecW = Math.max(1.2, zoom * 0.015);
  ...
  if (modoTecnico) {
    return (
      <g opacity={op} onMouseDown={onMouseDown} style={{ cursor: cur }}>
        ...
        {/* Alma doble (reforzada) */}
        <line x1={pL.x} y1={pL.y - g * 0.12} x2={pR.x} y2={pR.y - g * 0.12}
        ...
        <line x1={pR.x} y1={pR.y - mH / 2} x2={pR.x} y2={pR.y + mH / 2}
          stroke={sc} strokeWidth={tecW} />
        {/* Etiqueta T14 en modo técnico */}
        {showLabel && w > 30 && (
          <text x={pL.x + w / 2} y={pL.y - alaH - 2}
            fontSize={Math.max(5, zoom * 0.04)} fill={sc} textAnchor="middle"
            fontFamily="monospace" opacity="0.5">T14</text>
        )}
      </g>
    );
  }
  ...
  return (
    <g opacity={op} onMouseDown={onMouseDown} style={{ cursor: cur }}>
      ...
      {/* ═══ Alma principal (más gruesa, doble línea visual) ═══ */}
      <line x1={pL.x} y1={pL.y} x2={pR.x} y2={pR.y}
        stroke={sc} strokeWidth={g} strokeLinecap="butt" />
      {/* Segunda línea interior (efecto reforzado T14) */}
      <line x1={pL.x} y1={pL.y} x2={pR.x} y2={pR.y}
        stroke="#fff" strokeWidth={g * 0.15} strokeLinecap="butt" opacity="0.2" />
      {/* Overlay galvanizado */}
      <line x1={pL.x} y1={pL.y} x2={pR.x} y2={pR.y}
      ...
      {/* ═══ Alas U extremos (más altas que HorizontalU) ═══ */}
      <line x1={pL.x} y1={pL.y + g * 0.3} x2={pL.x} y2={pL.y - alaH}
        stroke={sc} strokeWidth={Math.max(1.2, zoom * 0.016)} />
      <line x1={pR.x} y1={pR.y + g * 0.3} x2={pR.x} y2={pR.y - alaH}
        stroke={sc} strokeWidth={Math.max(1.2, zoom * 0.016)} />
      ...
      {/* Etiqueta "T14" + largo (zoom medio+) */}
      {showLabel && w > 35 && (
        <text x={pL.x + w / 2} y={pL.y - alaH - 3}
          fontSize={Math.max(6, zoom * 0.045)} fill={sc} textAnchor="middle"
          fontFamily="monospace" opacity="0.5">T14 {largo.toFixed(2)}m</text>
      )}
    </g>
  );
}
