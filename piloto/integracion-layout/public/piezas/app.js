import { convertirAPiezaCatalogo, guardarPiezaImportada, cargarPiezasImportadas, validarPiezaImportada } from '../compartido/importador.js';
// ═══════════════════════════════════════════════════════════════════
...
let reviewedPiece = null;
const review = document.getElementById('layoutReview');
const status = document.getElementById('layoutStatus');
document.getElementById('addLayout').onclick = () => {
  const piece = buildPieceJson();
  const errors = [...validatePiece(piece), ...validarPiezaImportada(piece)];
  for (const key of ['largo','alto','profundidad']) if (!Number.isFinite(piece.dimensiones[key]) || piece.dimensiones[key] < 0) errors.push(`Medida inválida: ${key}`);
  if (!Number.isFinite(piece.despiece.pesoKg)) errors.push('Peso inválido');
  if (errors.length) { status.textContent = `Revisá: ${[...new Set(errors)].join(', ')}.`; return; }
  reviewedPiece = structuredClone(piece);
  const existing = cargarPiezasImportadas().some(p => p.id === piece.id);
  document.getElementById('layoutSummary').textContent = `${piece.nombre} · ${piece.id} · ${piece.dimensiones.largo} × ${piece.dimensiones.alto} × ${piece.dimensiones.profundidad} m · ${piece.despiece.pesoKg} kg`;
  document.getElementById('layoutDuplicate').textContent = existing ? 'Este ID ya existe: se actualizará la pieza del catálogo. Las piezas ya colocadas en planos conservarán sus datos.' : 'Se agregará a Piezas importadas. El plano actual no cambia.';
  document.getElementById('layoutConfirm').textContent = existing ? 'Actualizar pieza del catálogo' : 'Agregar pieza';
  const preview = els.canvas.cloneNode(true);
  preview.removeAttribute('id');
  preview.querySelectorAll('[id]').forEach(el => el.removeAttribute('id'));
  preview.style.cssText = 'width:100%;height:200px;pointer-events:none';
  document.getElementById('layoutPreview').replaceChildren(preview);
  review.showModal();
};
document.getElementById('layoutCancel').onclick = () => { reviewedPiece = null; review.close(); };
document.getElementById('layoutConfirm').onclick = () => {
  if (!reviewedPiece) return;
  try {
    guardarPiezaImportada(convertirAPiezaCatalogo(reviewedPiece));
    status.textContent = `✓ ${reviewedPiece.nombre} agregada. Volvé a la pestaña de Layout: está en Piezas importadas.`;
    reviewedPiece = null;
    review.close();
  } catch(error) { document.getElementById('layoutDuplicate').textContent = error.message; }
};
