...
export default function HorizontalU({ pieza, worldToScreen, zoom, sc, op, cur, seleccionada, onMouseDown, modoTecnico }) {
  ...
  return (
    <g opacity={op} onMouseDown={onMouseDown} style={{ cursor: cur }}>
      ...
      {modoTecnico ? <>
        <line x1={pL.x} y1={pL.y - mH / 2} x2={pL.x} y2={pL.y + mH / 2} stroke={sc} strokeWidth={Math.max(1, zoom * 0.012)} />
        <line x1={pR.x} y1={pR.y - mH / 2} x2={pR.x} y2={pR.y + mH / 2} stroke={sc} strokeWidth={Math.max(1, zoom * 0.012)} />
      </> : <>
